...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ErrorMessage> dataFormatExceptionHandler(
      HttpServletRequest request, Exception exception) {
    logger.warn("数据格式错误",
        request.getRemoteHost(), request.getRequestURL(), exception.getMessage());
  ...
  public ResponseEntity<ErrorMessage> notSupportedExceptionException(
      HttpServletRequest request, Exception exception) {
    logger.error("当前请求不支持",
        request.getRemoteHost(), request.getRequestURL(), exception.getMessage());
    return this.buildResponse(exception, HttpStatus.NOT_ACCEPTABLE);
  }
   ...
   * @return 异常处理器
   */
  @ExceptionHandler(value = ServiceNotRegisteredException.class)
  public ResponseEntity<ErrorMessage> serviceNotRegisteredExceptionHandler(
      HttpServletRequest request, Exception exception) {
    logger.error("未找到对应的注册服务",
        request.getRemoteHost(), request.getRequestURL(), exception.getMessage());
    exception.printStackTrace();
    return this.buildResponse(exception, HttpStatus.NOT_IMPLEMENTED);
  }

  @ExceptionHandler(value = TimelinessException.class)
  public ResponseEntity<ErrorMessage> timelinessExceptionHandler(
      HttpServletRequest request, Exception exception) {
    logger.error("请求的数据超出了时效性",
        request.getRemoteHost(), request.getRequestURL(), exception.getMessage());
    exception.printStackTrace();
    return this.buildResponse(exception, HttpStatus.PRECONDITION_FAILED);
  }
