...
import {Assert, isNotNullOrUndefined, UnknownProperty} from '@yunzhi/utils';
...
export class CommonService {
   ...
   * @param format 格式
   */
  public static convertToDatetimeString(timestamp: number, format: string): string {
    return this.datePipe.transform(new Date(timestamp).toISOString(), format, 'GMT+8:00');
  }
   ...
   */
  public static convertToRouteParams(params: {[header: string]: string | string[] | number | number[] | boolean | null | undefined;})
  ...
  /**
   * 路由后退
   * @param url 回退路径，仅当route不为null时生效
   * @param route 路由
   */
   ...
   * @return close 关闭弹窗 currentProcessSubject 用于发送当前进度的数据源
   */
  loading(title = '请稍后', description = '', currentProcess = 0, maxStack = 1000):
    {close: () => void, updateProgress: (progress: number) => void} {
    const html = `${description}<b>${currentProcess}</b>%`;
    const currentProcessSubject = new Subject<number>();
    currentProcessSubject.asObservable().subscribe((tip) => {
      Swal.getHtmlContainer().querySelector('b').textContent = tip.toString();
    });
    ...
    return {
      ...
      updateProgress: (progress: number) => {
        currentProcessSubject.next(progress)
      }
    };
  }
